...
st.caption("⚡ Реальное время • Обновление каждые 1 секунду • Премиум дашборд")
...
def load_orders():
    try:
        ...
        df = pd.DataFrame(columns=["Дата", "Приоритет", "Категория", "Заголовок", "Бюджет", "Предложений", "Описание", "Ссылка"])

    if "Приоритет" not in df.columns:
        df["Приоритет"] = "📌 Обычный"
